...
class userDataByid(Resource):
    @jwt_required()
    @cache.cached(timeout=30, key_prefix=lambda: f"userdata_{get_jwt_identity()}")
    def get(self):
...
class campaignById(Resource):
    @jwt_required()
    @cache.cached(timeout=30, key_prefix=lambda: f"campaign_{get_jwt_identity()}")
    def get(self):
        current_user = get_jwt_identity()
...
class addAccount(Resource):
    @jwt_required()
    @cache.cached(timeout=30, key_prefix=lambda: f"account_{get_jwt_identity()}")
    def get(self):
        current_user = get_jwt_identity()
...
class Signatories(Resource):    
    @jwt_required()
    @cache.cached(timeout=30, key_prefix=lambda: f"signatories_{get_jwt_identity()}")
    def get(self):
        current_user = get_jwt_identity()
        existing_organisation = Organisation.query.filter_by(id=current_user).first()
        if not existing_organisation:
...
def collection_webhook():
    try:
        ...
        if state == "COMPLETE":
            ...
            #send to pocket 
            app_commission= round((float(donated_amount) * 0.04),2)  
        ...
        return (f"Collecting hook error occurred: {e}")
...
def send_money_webhook():
    try:
        ...
        logging.error(e)
        return (f"Send money hook error occurred: {e}")
...
@jwt_required()
@cache.cached(timeout=30, key_prefix=lambda: f"withdrawpdf_{get_jwt_identity()}")
def withdraw_pdf():
    current_user = get_jwt_identity()
    existing_org = Organisation.query.filter_by(id=current_user).first()
    if not existing_org:
    ...
    try:
        ...
        pdf.setTitle("MsaadaFund/withdrawals")
        pdf.setFont("Helvetica", 12)

        logo_url = "https://res.cloudinary.com/dml7sp2zm/image/upload/e_improve,e_sharpen/v1721687108/applogo_wrpxz6.jpg"
        ...
        def draw_logo(pdf):
            ...
            pdf.drawImage(tmp_file.name, x=0.5 * inch, y=10.8 * inch, width=logo_width, height=logo_height)            

        # Add table headers
        draw_logo(pdf)
        pdf.setFont("Helvetica-Bold", 13)
        pdf.drawString(0.5 * inch, 10.4 * inch, f"Withdrawals for {existing_org.orgName}")
        y = 10 * inch
        pdf.setFont("Helvetica-Bold", 11)
        pdf.drawString(0.5 * inch, y, "SNo.")
        ...
        pdf.drawString(1 * inch, 0.25 * inch, "Generated by @MsaadaFund platform")
                        
        # You need to generate the PDF before returning it
        pdf.save()
        buffer.seek(0)
        
        return Response(buffer.getvalue(), mimetype="application/pdf", headers={"Content-Disposition": "attachment;filename=withdrawals.pdf"})
...
def get():
    ...
    try:
        ...
        response = jsonify({"message":response_dict}),200
        return response
    except  Exception as e:
...
# @jwt_required()
# @cache.cached(timeout=30, key_prefix=lambda: f"transpdf_{get_jwt_identity()}")
...
#                 y -= 0.2 * inch  # Move down for the next row
#         else:
...
@jwt_required()
# @cache.cached(timeout=30, key_prefix=lambda: f"donpdf_{get_jwt_identity()}")
def get_org_donations_pdf():
    ...
    try:
        ...
        # Set PDF title and other metadata
        pdf.setTitle("MsaadaFund/Donation")
        ...
        draw_logo(pdf)
        pdf.setFont("Helvetica-Bold", 13)
        pdf.drawString(0.5 * inch, 10.4 * inch, f"Donations for {existing_org.orgName}")
        y = 10 * inch
        pdf.setFont("Helvetica-Bold", 11)
        pdf.drawString(0.5 * inch, y, "SNo.")
        pdf.drawString(1 * inch, y, "Invoice ID")
        pdf.drawString(2 * inch, y, "Name")
        pdf.drawString(3.2 * inch, y, "Campaign")
        pdf.drawString(5.2 * inch, y, "Amount")
        pdf.drawString(6.5 * inch, y, "Method")
        
        # Initialize counter for number of donations per page
        donations_per_page = 30
        donation_count = 0
        ...
        for index, donation in enumerate(all_donations, start=1):
            ...
            pdf.drawString(0.5 * inch, y, str(index))
            pdf.drawString(1 * inch, y, invoice_id)
            pdf.drawString(2 * inch, y, donation.donor_name if donation.donor_name else f"{User.query.filter_by(id=donation.user_id).first().firstName} {User.query.filter_by(id=donation.user_id).first().lastName}")
            pdf.drawString(3.2 * inch, y, campaign_name[0])  # Limit campaign name length
            pdf.drawString(5.2 * inch, y, f"KSH {donation.amount:.2f}")
            pdf.drawString(6.5 * inch, y, donation.method)
        ...
        return Response(buffer.getvalue(), mimetype="application/pdf", headers={"Content-Disposition": "attachment;filename= contributions.pdf"})
